...
import gr.uom.java.xmi.diff.RenameLocalVariable;
...
//			String folder ="/Users/matin/Repos/RefactoringDatasets";
			...
			try (Repository rep = gitService.cloneIfNotExists(folder,
				...
				if (m.ignoreNonSpecifiedCommits) {
					...
					for (String commitId : m.getCommits()) {
						// if
						// (commitId.equals("031c1d91ccff9cf437af613145b69f91f9c4655b")
						// ||
...
//		RefactoringPopulator.save();
	...
	private List<String> normalize(String refactoring) {
		RefactoringType refType = RefactoringType.extractFromDescription(refactoring);
		if (!refType.name().toLowerCase().contains("local"))
	...
	public class ProjectMatcher extends RefactoringHandler {
		...
		@Override
		public void handle(String curRevision, List<Refactoring> refactorings) {
			// refactorings= filterRefactoring(refactorings);
			for (Refactoring refactoring : refactorings) {
				if (refactoring.toString().contains("Rename Local Variable")){
					RefactoringPopulator.updateIfExistRefactoring(curRevision, refactoring.toString(),((RenameLocalVariable)refactoring).getOperation().getContainerClass().getSourceFile());
				RefactoringPopulator.addRefactoring(curRevision, refactoring.toString(),((RenameLocalVariable)refactoring).getOperation().getContainerClass().getSourceFile());}
				System.out.println(refactoring.toString());
			}
			CommitMatcher matcher;
			commitsCount++;
			String commitId = curRevision; // curRevision.getId().getName();
				...
				if (matcher.error != null) {
					...
					if (!matcher.notExpected.isEmpty()) {
						...
						for (String ref : matcher.notExpected) {
							...
							System.out.println(RefactoringPopulator.getCommentIfExists(entry.getKey(), ref)+ "  \n" + ref);
