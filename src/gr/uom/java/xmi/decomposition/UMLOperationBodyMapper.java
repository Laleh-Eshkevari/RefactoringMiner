...
public class UMLOperationBodyMapper implements Comparable<UMLOperationBodyMapper> {
	...
	private List<AbstractCodeMapping> mappingsDiscardCandidate;
	...
	public UMLOperationBodyMapper(UMLOperation operation1, UMLOperation operation2) {
		...
		this.mappingsDiscardCandidate = new ArrayList<>();
	}
	...
	public void addAdditionalMapper(UMLOperationBodyMapper mapper) {
		...
		// which are less similar than the mappings of the mapper passed as
		// parameter
	}
	...
	private void initialize() {
		...
		// System.out.println("HERE" + Arrays.toString((StackTraceElement[])
		// Thread.currentThread().getStackTrace()).replace(",","\n"));
	...
	public UMLOperationBodyMapper(UMLOperationBodyMapper operationBodyMapper, UMLOperation addedOperation,
		...
		if (addedOperationBody != null) {
			...
			// TODO remove non-mapped inner nodes from T1 corresponding to
			// mapped expressions

			// remove the leaves that were mapped with replacement, if they are
			// not mapped again for a second time
			leaves1.removeAll(addedLeaves1);
			// remove the innerNodes that were mapped with replacement, if they
			// are not mapped again for a second time
	...
	public UMLOperationBodyMapper(UMLOperation removedOperation, UMLOperationBodyMapper operationBodyMapper,
		...
		if (removedOperationBody != null) {
			...
			// adding leaves that were mapped with replacements or are inexact
			// matches
			...
			if (!parameterToArgumentMap.isEmpty()) {
				// check for temporary variables that the argument might be
				// assigned to
				for (StatementObject leave2 : leaves2) {
			...
			// adding innerNodes that were mapped with replacements or are
			// inexact matches
	...
	public List<AbstractCodeMapping> getDiscardedMappings() {
		initialize();
		return this.mappingsDiscardCandidate;
	}
	...
	private boolean countableStatement(AbstractCodeFragment fragment) {
		...
		// and conditionals with only one statement in their body
		...
		return !statement.equals("{") && !statement.startsWith("catch(") && !statement.startsWith("case ")
				&& !statement.startsWith("default :") && !statement.startsWith("return true")
				&& !statement.startsWith("return false") && !statement.startsWith("return this")
				&& !statement.startsWith("return null") && !statement.startsWith("return;");
	}
	...
	public void processInnerNodes(List<CompositeStatementObject> innerNodes1,
			List<CompositeStatementObject> innerNodes2, Map<String, String> parameterToArgumentMap) {
		// exact string+depth matching - inner nodes
		for (ListIterator<CompositeStatementObject> innerNodeIterator1 = innerNodes1.listIterator(); innerNodeIterator1
				.hasNext();) {
			...
			for (ListIterator<CompositeStatementObject> innerNodeIterator2 = innerNodes2
					.listIterator(); innerNodeIterator2.hasNext();) {
				...
				if ((statement1.getString().equals(statement2.getString())
						...
						&& statement1.getDepth() == statement2.getDepth() && (score > 0 || Math
								.max(statement1.getStatements().size(), statement2.getStatements().size()) == 0)) {
					CompositeStatementObjectMapping mapping = new CompositeStatementObjectMapping(statement1,
							statement2, operation1, operation2, score);
		...
		for (ListIterator<CompositeStatementObject> innerNodeIterator1 = innerNodes1.listIterator(); innerNodeIterator1
			...
			for (ListIterator<CompositeStatementObject> innerNodeIterator2 = innerNodes2
				...
				double score = compositeChildMatchingScore(statement1, statement2);
				if (replacements != null && (score > 0
						|| Math.max(statement1.getStatements().size(), statement2.getStatements().size()) == 0)) {
					CompositeStatementObjectMapping mapping = new CompositeStatementObjectMapping(statement1,
							statement2, operation1, operation2, score);
					for (Replacement replacement : replacements) {
						replacement.setCodeMapping(mapping);
					}
	...
	public void processLeaves(List<? extends AbstractCodeFragment> leaves1,
			List<? extends AbstractCodeFragment> leaves2, Map<String, String> parameterToArgumentMap) {
		// exact string+depth matching - leaf nodes
		for (ListIterator<? extends AbstractCodeFragment> leafIterator1 = leaves1.listIterator(); leafIterator1
				.hasNext();) {
			AbstractCodeFragment leaf1 = leafIterator1.next();
			TreeSet<LeafMapping> mappingSet = new TreeSet<LeafMapping>();
			for (ListIterator<? extends AbstractCodeFragment> leafIterator2 = leaves2.listIterator(); leafIterator2
					.hasNext();) {
		...
		for (ListIterator<? extends AbstractCodeFragment> leafIterator1 = leaves1.listIterator(); leafIterator1
			...
			if (!mappingSet.isEmpty()) {
				LeafMapping minStatementMapping = mappingSet.first();
//				AbstractCodeFragment tmpLeaf1 = minStatementMapping.getFragment1();
//				for (ListIterator<? extends AbstractCodeFragment> tmPLeafIterator2 = leaves2
//						.listIterator(); tmPLeafIterator2.hasNext();) {
//					AbstractCodeFragment leaf2 = tmPLeafIterator2.next();
//
//					Set<Replacement> replacements = findReplacementsWithExactMatching(tmpLeaf1, leaf2,
//							parameterToArgumentMap);
//					if (replacements != null) {
//						LeafMapping mapping = new LeafMapping(tmpLeaf1, leaf2, operation1, operation2);
//						mapping.addReplacements(replacements);
//						for (Replacement replacement : replacements) {
//							replacement.setCodeMapping(mapping);
//						}
...
//				}
				...
				if (mappingSet.higher(minStatementMapping) != null)
					minStatementMapping.setDiscardedMatch(mappingSet.lower(minStatementMapping));
				for (LeafMapping leafMapping : mappingSet) {
					if (!minStatementMapping.equals(leafMapping))
						mappingsDiscardCandidate.add(leafMapping);
				}
				mappings.add(minStatementMapping);
				leaves2.remove(minStatementMapping.getFragment2());
				leafIterator1.remove();
	...
	private Set<Replacement> findReplacementsWithExactMatching(AbstractCodeFragment statement1,
			AbstractCodeFragment statement2, Map<String, String> parameterToArgumentMap) {
		List<VariableDeclaration> variableDeclarations1 = new ArrayList<VariableDeclaration>(
				statement1.getVariableDeclarations());
		List<VariableDeclaration> variableDeclarations2 = new ArrayList<VariableDeclaration>(
				statement2.getVariableDeclarations());
		// for (String num : statement1.getNumericLiterals()) {
		// if()
		// }
		...
		// ignore the variables in the intersection that also appear with
		// "this." prefix in the sets of variables
		...
		Map<String, OperationInvocation> methodInvocationMap1 = new LinkedHashMap<String, OperationInvocation>(
				statement1.getMethodInvocationMap());
		Map<String, OperationInvocation> methodInvocationMap2 = new LinkedHashMap<String, OperationInvocation>(
		...
		for (String methodInvocation1 : methodInvocationMap1.keySet()) {
			if ((methodInvocation1 + ";\n").equals(statement1.getString())
					|| methodInvocation1.equals(statement1.getString())
		...
		for (String methodInvocation2 : methodInvocationMap2.keySet()) {
			if ((methodInvocation2 + ";\n").equals(statement2.getString())
					|| methodInvocation2.equals(statement2.getString())
		...
		// replace variables with the corresponding arguments in method
		// invocations
		for (String parameter : parameterToArgumentMap.keySet()) {
			String argument = parameterToArgumentMap.get(parameter);
		...
		for (String objectCreation1 : creationMap1.keySet()) {
			if ((objectCreation1 + ";\n").equals(statement1.getString())
					|| objectCreation1.equals(statement1.getString())
		...
		for (String objectCreation2 : creationMap2.keySet()) {
			if ((objectCreation2 + ";\n").equals(statement2.getString())
					|| objectCreation2.equals(statement2.getString())
		...
		// (double)StringDistance.editDistance(statement1.getString(),
		// statement2.getString())/(double)Math.max(statement1.getString().length(),
		// statement2.getString().length());
		Set<Replacement> replacements = new LinkedHashSet<Replacement>();
		...
		for (String type1 : types1) {
			...
			for (String type2 : types2) {
				...
				if (distanceRaw >= 0 && distanceRaw < initialDistanceRaw) {
					...
					double distancenormalized = (double) distanceRaw
							/ (double) Math.max(temp.length(), argumentizedString2.length());
			...
			if (!replacementMap.isEmpty()) {
				...
				replacements.add(replacement);
				argumentizedString1 = argumentizedString1.replaceAll(Pattern.quote(replacement.getBefore()),
						Matcher.quoteReplacement(replacement.getAfter()));
		...
		for (String infixOperator1 : infixOperators1) {
			...
			for (String infixOperator2 : infixOperators2) {
				String temp = argumentizedString1.replaceAll(Pattern.quote(infixOperator1),
						Matcher.quoteReplacement(infixOperator2));
				int distanceRaw = StringDistance.editDistance(temp, argumentizedString2, minDistance);
				if (distanceRaw >= 0 && distanceRaw < initialDistanceRaw) {
					minDistance = distanceRaw;
		...
		if (initialDistanceRaw > 0) {
			for (String s1 : variablesAndMethodInvocations1) {
				...
				for (String s2 : variablesAndMethodInvocations2) {
					String temp = performReplacement(argumentizedString1, argumentizedString2, s1, s2,
							variablesAndMethodInvocations1, variablesAndMethodInvocations2);
					int distanceRaw = StringDistance.editDistance(temp, argumentizedString2, minDistance);
					// double distance =
					// (double)StringDistance.editDistance(temp,
					// statement2.getString())/(double)Math.max(temp.length(),
					// statement2.getString().length());
					if (distanceRaw >= 0 && distanceRaw < initialDistanceRaw) {
						minDistance = distanceRaw;
						...
						if (variables1.contains(s1) && variables2.contains(s2) && variablesStartWithSameCase(s1, s2)) {
							...
							replacement = new VariableReplacementWithMethodInvocation(s1, s2,
									methodInvocationMap2.get(s2));
				...
				if (!replacementMap.isEmpty()) {
					Replacement replacement = replacementMap.firstEntry().getValue();
					replacements.add(replacement);
					argumentizedString1 = performReplacement(argumentizedString1, argumentizedString2,
							replacement.getBefore(), replacement.getAfter(), variablesAndMethodInvocations1,
							variablesAndMethodInvocations2);
					initialDistanceRaw = StringDistance.editDistance(argumentizedString1, argumentizedString2);
					if (replacementMap.firstEntry().getKey() == 0) {
						break;
					}
		...
		for (String creation1 : creations1) {
			...
			for (String creation2 : creations2) {
				String temp = argumentizedString1.replaceAll(Pattern.quote(creation1),
						Matcher.quoteReplacement(creation2));
				int distanceRaw = StringDistance.editDistance(temp, argumentizedString2, minDistance);
				if (distanceRaw >= 0 && distanceRaw < initialDistanceRaw) {
					minDistance = distanceRaw;
		...
		if (!containsMethodInvocationReplacement(replacements)) {
			for (String stringLiteral1 : stringLiterals1) {
				...
				for (String stringLiteral2 : stringLiterals2) {
					String temp = argumentizedString1.replaceAll(Pattern.quote(stringLiteral1),
							Matcher.quoteReplacement(stringLiteral2));
					int distanceRaw = StringDistance.editDistance(temp, argumentizedString2, minDistance);
					if (distanceRaw >= 0 && distanceRaw < initialDistanceRaw) {
						minDistance = distanceRaw;
		...
		for (Replacement replacement : replacements) {
			s1 = performReplacement(s1, s2, replacement.getBefore(), replacement.getAfter(),
					variablesAndMethodInvocations1, variablesAndMethodInvocations2);
		...
		if (!anonymousClassDeclarations1.isEmpty() && !anonymousClassDeclarations2.isEmpty()
			...
			for (int i = 0; i < anonymousClassDeclarations1.size(); i++) {
				...
				if (indexOfAnonymousClassDeclaration1 != -1 && indexOfAnonymousClassDeclaration2 != -1) {
					String statementWithoutAnonymous1 = statement1.getString().substring(0,
							indexOfAnonymousClassDeclaration1);
					String statementWithoutAnonymous2 = statement2.getString().substring(0,
							indexOfAnonymousClassDeclaration2);
					if (statementWithoutAnonymous1.equals(statementWithoutAnonymous2)) {
						int editDistance = StringDistance.editDistance(anonymousClassDeclaration1,
								anonymousClassDeclaration2);
						double distancenormalized = (double) editDistance / (double) Math
								.max(anonymousClassDeclaration1.length(), anonymousClassDeclaration2.length());
						if (distancenormalized < MAX_ANONYMOUS_CLASS_DECLARATION_DISTANCE) {
							Replacement replacement = new AnonymousClassDeclarationReplacement(
									anonymousClassDeclaration1, anonymousClassDeclaration2);
							replacements.add(replacement);
							return replacements;
						}
					}
		...
		if (invocationCoveringTheEntireStatement1 != null && invocationCoveringTheEntireStatement2 != null
				&& (invocationsWithIdenticalExpressions(invocationCoveringTheEntireStatement1,
						invocationCoveringTheEntireStatement2)
						|| invocationsWithIdenticalExpressionsAfterTypeReplacements(
								invocationCoveringTheEntireStatement1, invocationCoveringTheEntireStatement2,
								replacements))
				&& invocationCoveringTheEntireStatement1.getMethodName()
						.equals(invocationCoveringTheEntireStatement2.getMethodName())
				&& identicalArguments(invocationCoveringTheEntireStatement1, invocationCoveringTheEntireStatement2,
						variablesAndMethodInvocations1, variablesAndMethodInvocations2)) {
			return replacements;
		}
		// method invocation has been renamed but the expression and arguments
		// are identical
		if (invocationCoveringTheEntireStatement1 != null && invocationCoveringTheEntireStatement2 != null
				...
				&& !invocationCoveringTheEntireStatement1.getMethodName()
						.equals(invocationCoveringTheEntireStatement2.getMethodName())
				&& invocationCoveringTheEntireStatement1.getArguments()
						.equals(invocationCoveringTheEntireStatement2.getArguments())) {
			Replacement replacement = new MethodInvocationRename(invocationCoveringTheEntireStatement1.getMethodName(),
					invocationCoveringTheEntireStatement2.getMethodName(), invocationCoveringTheEntireStatement1,
					invocationCoveringTheEntireStatement2);
			replacements.add(replacement);
			return replacements;
		}
		// method invocation has been renamed and arguments changed, but the
		...
		if (invocationCoveringTheEntireStatement1 != null && invocationCoveringTheEntireStatement2 != null
				...
				&& (invocationsWithIdenticalExpressions(invocationCoveringTheEntireStatement1,
						...
						|| invocationsWithIdenticalExpressionsAfterTypeReplacements(
								invocationCoveringTheEntireStatement1, invocationCoveringTheEntireStatement2,
								replacements))
				&& invocationCoveringTheEntireStatement1.normalizedNameDistance(
						invocationCoveringTheEntireStatement2) <= UMLClassDiff.MAX_OPERATION_NAME_DISTANCE
				&& !identicalArguments(invocationCoveringTheEntireStatement1, invocationCoveringTheEntireStatement2,
						variablesAndMethodInvocations1, variablesAndMethodInvocations2)
				&& invocationCoveringTheEntireStatement1.getArguments().size() != invocationCoveringTheEntireStatement2
						.getArguments().size()) {
			Replacement replacement = new MethodInvocationRenameAndArgumentReplacement(
					invocationCoveringTheEntireStatement1.getMethodName(),
					invocationCoveringTheEntireStatement2.getMethodName(), invocationCoveringTheEntireStatement1,
					invocationCoveringTheEntireStatement2);
			replacements.add(replacement);
		...
		if (invocationCoveringTheEntireStatement1 != null && invocationCoveringTheEntireStatement2 != null
			...
			Set<String> argumentIntersection = new LinkedHashSet<String>(
					invocationCoveringTheEntireStatement1.getArguments());
			argumentIntersection.retainAll(invocationCoveringTheEntireStatement2.getArguments());
			if (!argumentIntersection.isEmpty() || invocationCoveringTheEntireStatement1.getArguments().size() == 0
					|| invocationCoveringTheEntireStatement2.getArguments().size() == 0
					|| (invocationCoveringTheEntireStatement1.getArguments().size() == 1
							&& invocationCoveringTheEntireStatement2.getArguments().size() == 1)) {
				Replacement replacement = new MethodInvocationArgumentReplacement(
						invocationCoveringTheEntireStatement1.getMethodName(),
						invocationCoveringTheEntireStatement2.getMethodName(), invocationCoveringTheEntireStatement1,
						invocationCoveringTheEntireStatement2);
				replacements.add(replacement);
		...
		if (!methodInvocations1.isEmpty() && invocationCoveringTheEntireStatement2 != null) {
			for (String methodInvocation1 : methodInvocations1) {
				...
				if ((invocationsWithIdenticalExpressions(operationInvocation1, invocationCoveringTheEntireStatement2)
						|| invocationsWithIdenticalExpressionsAfterTypeReplacements(operationInvocation1,
								invocationCoveringTheEntireStatement2, replacements))
						&& operationInvocation1.getMethodName()
								.equals(invocationCoveringTheEntireStatement2.getMethodName())
						&& !identicalArguments(operationInvocation1, invocationCoveringTheEntireStatement2,
								variablesAndMethodInvocations1, variablesAndMethodInvocations2)
						&& operationInvocation1.getArguments().size() != invocationCoveringTheEntireStatement2
								.getArguments().size()) {
					Set<String> argumentIntersection = new LinkedHashSet<String>(operationInvocation1.getArguments());
					argumentIntersection.retainAll(invocationCoveringTheEntireStatement2.getArguments());
					if (!argumentIntersection.isEmpty() || operationInvocation1.getArguments().size() == 0
							|| invocationCoveringTheEntireStatement2.getArguments().size() == 0) {
						Replacement replacement = new MethodInvocationArgumentReplacement(
								operationInvocation1.getMethodName(), operationInvocation1.getMethodName(),
								invocationCoveringTheEntireStatement1, invocationCoveringTheEntireStatement2);
						replacements.add(replacement);
						return replacements;
					}
		...
		if (invocationCoveringTheEntireStatement1 != null && invocationCoveringTheEntireStatement2 != null
				...
				&& !allArgumentsReplaced(invocationCoveringTheEntireStatement1, invocationCoveringTheEntireStatement2,
						replacements)) {
		...
		// check if the argument of the method call in the first statement is
		// returned in the second statement
		if (invocationCoveringTheEntireStatement1 != null && argumentizedString2.startsWith("return ")
				&& invocationCoveringTheEntireStatement1.getArguments().size() == 1 &&
				// length()-2 to remove ";\n" from the end of the return
				// statement, 7 to remove the prefix "return "
				equalsIgnoringExtraParenthesis(invocationCoveringTheEntireStatement1.getArguments().get(0),
						argumentizedString2.substring(7, argumentizedString2.length() - 2))) {
			Replacement replacement = new ArgumentReplacementWithReturnExpression(
					invocationCoveringTheEntireStatement1.getArguments().get(0),
					argumentizedString2.substring(7, argumentizedString2.length() - 2));
			replacements.add(replacement);
			return replacements;
		}
		// check if the argument of the method call in the second statement is
		// the right hand side of an assignment in the first statement
		if (invocationCoveringTheEntireStatement2 != null
				&& invocationCoveringTheEntireStatement2.getArguments().size() == 1 && argumentizedString1.contains("=")
				&& argumentizedString1.endsWith(";\n") &&
				// length()-2 to remove ";\n" from the end of the assignment
				// statement, indexOf("=")+1 to remove the left hand side of the
				// assignment
				equalsIgnoringExtraParenthesis(invocationCoveringTheEntireStatement2.getArguments().get(0),
						argumentizedString1.substring(argumentizedString1.indexOf("=") + 1,
								argumentizedString1.length() - 2))
				&& methodInvocationMap1.containsKey(invocationCoveringTheEntireStatement2.getArguments().get(0))) {
			Replacement replacement = new ArgumentReplacementWithRightHandSideOfAssignmentExpression(
					argumentizedString1.substring(argumentizedString1.indexOf("=") + 1,
							argumentizedString1.length() - 2),
		...
		if (!creations1.isEmpty() && creationCoveringTheEntireStatement2 != null) {
			for (String creation1 : creations1) {
				...
				if (objectCreation1.getType().equals(creationCoveringTheEntireStatement2.getType())
						...
						&& objectCreation1.getArguments().size() != creationCoveringTheEntireStatement2.getArguments()
								.size()) {
					...
					if (!argumentIntersection.isEmpty() || objectCreation1.getArguments().size() == 0
							|| creationCoveringTheEntireStatement2.getArguments().size() == 0) {
						Replacement replacement = new ObjectCreationArgumentReplacement(
								objectCreation1.getType().toString(),
								creationCoveringTheEntireStatement2.getType().toString(), objectCreation1,
	...
	private boolean identicalArguments(OperationInvocation invocationCoveringTheEntireStatement1,
			OperationInvocation invocationCoveringTheEntireStatement2, Set<String> variablesAndMethodInvocations1,
			Set<String> variablesAndMethodInvocations2) {
		...
		for (int i = 0; i < arguments1.size(); i++) {
			...
			if (!argument1.equals(argument2) && !variablesAndMethodInvocations1.contains(argument2)
					&& !variablesAndMethodInvocations2.contains(argument1))
				return false;
		}
		return true;
	}
	...
	private Replacement variableReplacementWithinMethodInvocations(String s1, String s2, Set<String> variables1,
	...
	private boolean allArgumentsReplaced(OperationInvocation invocation1, OperationInvocation invocation2,
	...
	private boolean invocationsWithIdenticalExpressions(OperationInvocation invocation1,
	...
	private boolean invocationsWithIdenticalExpressionsAfterTypeReplacements(OperationInvocation invocation1,
			OperationInvocation invocation2, Set<Replacement> replacements) {
		if (invocation1.getExpression() != null && invocation2.getExpression() != null) {
			...
			for (Replacement replacement : replacements) {
				if (replacement instanceof TypeReplacement) {
					invocationExpression1AfterReplacements = performReplacement(invocationExpression1AfterReplacements,
	...
	private String performReplacement(String completeString1, String completeString2, String subString1,
	...
	private double compositeChildMatchingScore(CompositeStatementObject composite1,
			CompositeStatementObject composite2) {
		...
		for (AbstractCodeMapping mapping : mappings) {
			if (composite1.getStatements().contains(mapping.getFragment1())
					&& composite2.getStatements().contains(mapping.getFragment2())) {
				mappedChildrenSize++;
			}
		}
